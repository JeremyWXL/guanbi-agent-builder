...
6. **看板适检（勾选后、确认点前静默执行）**：不是所有看板都适合组装成 data agent——适检把不合适的在投入学习前拦下来：
   ```bash
   python3 references/scripts/check_pages.py <pageId1> <pageId2> ...
   ```
   每张看板只调一次接口、并发执行（数秒出结果），输出三档：
   - ⛔ 不可用：内容取不到（移动轻应用/报告推送/填报页/无权限）、数据大屏/导航概览页、0 数据卡、数据集全部不可读
   - ⚠️ 有风险：数据卡重名多（按名找卡会歧义）、数据停更超阈值（默认 90 天，--stale-days 可调）、卡名锁死固定期间且无时间筛选器（一次性报告快照）、数据卡 <5 张（覆盖面窄）、名称疑似演示/验证/备份页
   - ✅ 适合：无风险信号
   有 ⛔/⚠️ 时用业务语言复述原因并给替换建议（如"这张是数据大屏，同主题的经营分析页更合适"），用户调整勾选后再进确认点；⚠️ 看板用户坚持保留的可以继续，但风险点要在第 2 步资产目录中标注跟踪
