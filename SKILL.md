...
**确认点**：把资产目录用业务语言讲给用户听："我从这 N 张看板里学到了这些分析能力：…这些理解对吗？有没有遗漏的重要分析角度？"用户确认/纠偏后进入第 3 步。
...
**确认点**：逐条展示规则清单请用户确认："我把刚才聊的口径整理成了 N 条规则，其中归因公式是：…您看有没有说错的？"**这一步不允许跳过**，口径错误是洞察质量的最大杀手。
...
4. 向用户展示摘要："分析助手会这样思考问题：先抓最大异常 → 量化影响 → 追到具体门店 → 再谈建议。报告会分成这几部分：…需要调整吗？"
...
1. 在 `~/.workbuddy/skills/agent-<场景名>/` 生成产物 skill 包（模板：`references/templates/agent-SKILL.md`）：
   ...
   ```
2. 若数据集有 SQL 直查能力，在助手 SKILL.md 中标注"SQL 直查触发条件"，并在 references 中保留 sql-guide.md
3. 告诉用户："您的分析助手已就绪。以后直接对它提问即可，比如：…（列 3 个第 5 步确认的典型问题）"
