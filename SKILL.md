...
快速模式（lite）播报用五步地图："第 N 步/共 5 步（快速模式）"：1 选定数据 → 2 看板学习 → 3 口径裁决 → 4 场景与框架 → 5 验收与交付。状态机仍按原八步记录（合并执行的步骤照常 confirm），只是对用户按五步播报。

## 搭建模式选择（preflight 通过后、第 1 步之前）

新搭建（非断点续建）时，给用户一次模式选择：

> "两种方式搭建：**快速模式**——约 15 分钟，从您的看板出发先跑起来，口径冲突我会请您拍板，其余按看板算法自动采纳，以后随时可以深化；**完整模式**——约 40-60 分钟，口径和维度逐条打磨，分析更准更稳。您选哪个？"

- 用户选快速模式：`python3 references/scripts/wizard_state.py init <工作目录> --mode lite`，然后按「快速模式（lite）分支」执行
- 用户选完整模式或不表态：默认完整模式（`init` 不带 --mode），按八步向导执行
- 快速模式**从看板开始**，不是数据集直通路径；数据集直通仍按第 1 步原触发条件兜底（看板全部 ⛔ / 没有看板 / 评分全低且用户暂不愿治理），两种模式下都可用
- 断点续建时不重新问模式，按 wizard-state.json 里的 mode 恢复
...
用户选择快速模式时执行本分支。裁剪原则：**砍确认深度，不砍产物结构，不砍质量命门**——交付包与完整版结构完全同构（含 memory/ 四文件），差别只在档案内容的确认深度。

### L1 选定数据（= 第 1 步全量）

动作与第 1 步完全一致：目录扫描 → 勾选 → **适检三档 + agent-ready 评分照跑** → selection.html 核对页 → 业务范围追问。提醒义务不破：⛔ 拦下并给替换建议；⚠️ / 评分「低」/ 🔴 红线的看板用业务语言复述风险（"这张看板里同一个指标有几种算法，学进去助手会给出矛盾答案"）并建议先治理——**用户坚持继续的可以继续**，风险点登记进资产目录跟踪。快速模式不降低知识源质量门槛的告知标准，只把选择权交给用户。

### L2 看板学习（= 第 2 步全量）

动作与第 2 步完全一致：parse_page（单次传入全部 pageId）→ sample_cards → validate_cards → check_formulas `--seed-metrics` → check_dims `--seed-dims`。**种子照常生成**——它是快速模式免逐条确认还能成稿的底稿，也是后续深化的起点。

### L3 口径裁决（= 第 4 步精简）

只保留三件事：
1. **冲突拍板组逐条裁决**（红线不破：禁止 AI 自行二选一）——formulas.json 的 conflicts 清单逐条请用户拍板
...
四类能力一次性介绍（问数/归因/异常/综合洞察），只问用户"最常被问到的 2-3 个分析问题"；examples.json 只固化这几题；insightThinking 用模板默认框架 + 诊断链方法论（诊断链是核心差异化资产，快速模式不裁）；输出模板用默认。

### L5 验收与交付（= 第 7+8 步合并）

每个场景 1 题实测（质量自检清单照常：数字有出处、合计闭环、归因走 attribute.py 禁止口算）；验收通过回填 examples.json 的 expect/answerPoints 并置 humanConfirmed。交付动作与第 8 步完全一致——交付包结构同构（SKILL.md + workbench.html + memory/ 四文件 + references 全套），`memory.py init` 照跑。交付话术补一句："这是快速模式交付的助手，口径按看板算法自动采纳的居多；以后想逐条打磨口径、让分析更准，跟我说「深化」就行。"

### 快速模式三条不破底线

1. 适检与评分的**提醒义务**不破（用户坚持可继续，但风险必须说透并登记）
2. 口径**冲突逐条裁决**不破（跳过 = 交付一个会给矛盾答案的助手）
3. **验收实测**不破（每场景至少 1 题真实取数验证；第 4、7 步在状态机里仍禁止 skipped）

## 深化通道（lite → 完整）

快速模式交付后，用户说"深化一下 / 逐条打磨口径 / 升级成完整版"时执行：

1. preflight 断点检测识别 wizard-state.json 的 `mode: "lite"` 且八步已全部完成时，会提示可深化；`wizard_state.py next` 同样提示
2. 续建直接回到**完整版第 4 步**：以交付包/工作目录已有的 metrics.json、dimensions.json 为底稿，按异议驱动逐条确认（共识组逐条过一遍、疑似口径问题补问、维度三件事拍板）——等于把快速模式跳过的确认深度补回来，双闸门重新校验
3. 再补完整版第 7 步：扩充 examples.json 题库（每场景补足测试题并验收回填）
4. 深化完成后把 businessKnowledge.md 顶部的"快速模式成稿"标注改为"已逐条确认"；`wizard_state.py set --step 4 --status confirmed` 等状态照常更新，无需改 mode 字段
