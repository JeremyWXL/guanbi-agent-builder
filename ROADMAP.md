...
- ~~businessKnowledge 结构化~~ —— ✅ 已完成（v3.2.0）：metrics.json 机器可读口径档案（OSI 对齐）+ check_metrics.py 校验闸门 + `--seed-metrics` 种子生成
1. **workbench.py 工程化**：拆分模板渲染/数据收集/服务层，对 collect()/check_staleness() 补最小单测（1340+ 行无测试是最大工程债；注意交付模型是单文件复制，拆分时需保持 `workbench.py` 单文件交付形态或同步改第 8 步交付清单）
